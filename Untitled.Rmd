...
p <- ggmap(get_googlemap(center = c(lon =  -97.733330, lat = 30.266666),
                         ...
                         maptype ='terrain',
                         color = 'color'))
map1 = p + geom_point(aes(x = lon, y = lat, color = percent_of_education), 
...
map2 = p + geom_point(aes(x = lon, y = lat, color = Median.Household.Income), 
                      data = parks_and_income, size = 3)+ xlab("Longitude") + ylab("Latitude") + ggtitle("Figure 1") +
  theme(plot.title = element_text(hjust = 0.5))
...
clust_totals = summarize(group_by(parks_and_income, clusters), total_parks = n(), acres = sum(acres)) %>%
  mutate(share_of_parks = total_parks/sum(total_parks)) %>%
  mutate(avg_acre = acres/total_parks)
```
